...
export default async function BrandModelList({brandSlug}: {brandSlug: string | undefined}) {
  ...
  return (
    ...
    {brand &&
    <>
    <div className="flex flex-col md:flex-row justify-between w-full bg-navy pt-6 px-12 md:pt-12 md:pl-48 md:pr-72 text-white font-bebas-neue">
      <div>
        <p className="text-red-500 text-xl font-bebas-neue">VEHICLE BRAND</p>
        <h1 className="text-2xl md:text-7xl font-bold md:mb-4 text-white">{brand.name}</h1>
        <div className="flex flex-row">
          <div className="flex w-max-w flex-col border-r-2 pr-4 md:pr-8">
            <h1 className="text-xl md:text-5xl">{brand._count.brandModels}</h1>
            <h1 className="text-md md:text-2xl">Models</h1>
          </div>
          <div className="flex flex-col pl-4 md:pl-8 border-r-2 pr-4 md:pr-8">
            <h1 className="text-xl md:text-5xl">{totalComplaints}</h1>
            <h1 className="text-md md:text-2xl">Complaints</h1>
          </div>
          <div className="flex flex-col px-4 md:px-8">
            <h1 className="text-xl md:text-5xl">{avgSeverity}</h1>
            <h1 className="text-md md:text-2xl">Severity</h1>
          </div>
        </div> 
      </div>
      <SlidingImage/>
    ...
    <div className="max-w-[800px] mx-auto px-4 my-8">
      {sorted.map(([type, brandModels]) => (
        <section key={type} className="flex flex-col">
          <h2 className="mb-2 text-2xl text-gray-500 font-bebas-neue">{CAR_TYPE_LABELS[type as CarType]}</h2>
          <div className='w-full grid grid-cols-2 md:grid-cols-3 gap-x-2 gap-y-2 mb-4'>
          {brandModels?.map((brandModel) => {
            ...
            return (
              <div key={brandModel.id} className="relative group overflow-hidden bg-white p-2 border border-border hover:border-red-500 rounded-lg cursor-pointer">
                <Link  href={`/${brand.slug}/${brandModel.slug}`} className='block text-xl font-bebas-neue'>          
                    {brandModel.name}         
                </Link>
