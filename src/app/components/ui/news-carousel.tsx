        ...
        <div className="embla__viewport" ref={emblaRef}>
          <div className="embla__container" id={carouselId}>
            {newsArticles?.map((article) => (
              <div className="embla__slide grid grid-cols-1 md:grid-cols-2 gap-y-2 md:gap-y-0 md:gap-x-8 md:px-12 md:py-4 overflow-hidden content-start" key={article.uuid}>
                ...
                <div className='md:py-16 md:px-4 flex flex-col items-start text-left p-2 justify-start gap-y-2'>
                  <h2 className='text-lg md:text-2xl font-bold'>{article.title}</h2>
                  <h3 className='text-md md:text-lg text-pretty text-gray-600'>{article.incipit}</h3>
