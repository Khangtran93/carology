...
export default async function Page({params}: {params: Promise<{brandSlug:string}>}) {
  ...
  return (
    <>
    {brand ? 
    <div className='flex flex-col max-w-[1300px] mx-auto pb-24 items-center'>
      <div>
      ...
      {brand.brandModels.map((brandModel, index) => (
       <Link key={index} href={`/brand/${brand.slug}/${brandModel.slug}`} className='p-2 bg-gray-200 text-black border border-gray-400 rounded-lg m-2 w-[40%] md:w-[45%]' > 
          <li >
            {brandModel.name}
          </li>
        </Link>
      ...
      {/* <Link href={`/brand/${}`}>
        <Button label='Add Complaint' className='mt-4 '/>
      </Link> */}
